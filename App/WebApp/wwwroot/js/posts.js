function getPopularPosts() {
    ...
    document.getElementById("feed-post-area").innerHTML="";
    $.ajax({
        url: '/Post/GetPopularPosts',
        type: "get",
        async: false,
        success: function (result) {
            if (result.length>0) {
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    if (checkUserAnsweredPost.length > 0) {
                        ...
                        postToAdd += "<div class=\"feed-post-answered-count-text-wrapper\">\n" +
                            ...
                            "                </div>\n" +
                            "            </div>";
                        postToAdd += `   <a href='question/${encodeURIComponent(result[i].post.title)}' class=\"feed-post-show-all-comments\">\n` +
...
function getPostsByAnsweredByUser() {
    $.ajax({
        url: '/Home/ResetPageNumber',
        type: "post",
        async:false,
    ...
    document.getElementById("feed-post-area").innerHTML="";
    $.ajax({
        url: '/Post/GetPostsByAnsweredByUser',
        type: "get",
        async: false,
        success: function (result) {
            if (result.length>0) {
